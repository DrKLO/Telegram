    ...
    uint16_t* magnU16)
{
    ...
    // Get input energy
    inst->energyIn = WebRtcSpl_Energy(winData, inst->anaLen, &inst->scaleEnergyIn);
    ...
    short* const* outFrame)
{
    ...
    if (fwrite(outframe, sizeof(short),
            inst->blockLen10ms, inst->outfile)
        != inst->blockLen10ms)
    {
        RTC_NOTREACHED();
    }
    ...
    if (num_bands > 1)
    {
        ...
        for (i = 0; i < num_high_bands; ++i)
        {
            for (j = 0; j < inst->blockLen10ms; j++)
            {
                outFrameHB[i][j] = (int16_t)((gainTimeDomainHB * inst->dataBufHBFX[i][j]) >> 14); // Q0
            }
        }
    } // end of H band gain computation
}
