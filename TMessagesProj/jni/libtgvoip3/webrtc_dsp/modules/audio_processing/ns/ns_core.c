...
int WebRtcNs_InitCore(NoiseSuppressionC* self, uint32_t fs)
{
    ...
    for (i = 0; i < SIMULT; i++)
    {
        self->counter[i] = (int)floor((float)(END_STARTUP_LONG * (i + 1)) / (float)SIMULT);
    }
    ...
    float* noise)
{
    ...
    for (s = 0; s < SIMULT; s++)
    {
        ...
        for (i = 0; i < self->magnLen; i++)
        {
            ...
            {
                self->density[offset + i] = ((float)self->counter[s] * self->density[offset + i] + 1.f / (2.f * WIDTH)) / (float)(self->counter[s] + 1);
            }
...
static void FeatureParameterExtraction(NoiseSuppressionC* self, int flag)
{
    ...
    if (flag == 1)
    {
        ...
        if ((fabs(posPeak2SpecFlat - posPeak1SpecFlat) < self->featureExtractionParams.limitPeakSpacingSpecFlat) && (weightPeak2SpecFlat > self->featureExtractionParams.limitPeakWeightsSpecFlat * weightPeak1SpecFlat))
        {
        ...
        // Merge the two peaks if they are close.
        if ((fabs(posPeak2SpecDiff - posPeak1SpecDiff) < self->featureExtractionParams.limitPeakSpacingSpecDiff) && (weightPeak2SpecDiff > self->featureExtractionParams.limitPeakWeightsSpecDiff * weightPeak1SpecDiff))
        {
