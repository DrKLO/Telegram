...
struct ExtendedFilter
{
    ...
    explicit ExtendedFilter(bool enabled)
        : enabled(enabled)
    {
    }
...
struct RefinedAdaptiveFilter
{
    ...
    explicit RefinedAdaptiveFilter(bool enabled)
        : enabled(enabled)
    {
    }
    static const ConfigOptionID identifier = ConfigOptionID::kAecRefinedAdaptiveFilter;
    bool enabled;
};
...
struct DelayAgnostic
{
    ...
    explicit DelayAgnostic(bool enabled)
        : enabled(enabled)
    {
    }
...
struct ExperimentalNs
{
    ...
    explicit ExperimentalNs(bool enabled)
        : enabled(enabled)
    {
    }
...
public:
    ...
    private:
        RuntimeSetting(Type id, float value)
            : type_(id)
            , value_(value)
        {
        }
    ...
    static constexpr size_t kNumNativeSampleRates = arraysize(kNativeSampleRatesHz);
    static constexpr int kMaxNativeSampleRateHz = kNativeSampleRatesHz[kNumNativeSampleRates - 1];
...
public:
    ...
    {
        return sample_rate_hz_ == other.sample_rate_hz_ && num_channels_ == other.num_channels_ && has_keyboard_ == other.has_keyboard_;
    }
...
private:
    ...
    {
        return static_cast<size_t>(AudioProcessing::kChunkSizeMs * sample_rate_hz / 1000);
    }
