...
#define RTC_HISTOGRAM_COMMON_BLOCK(constant_name, sample,                                                          \
    ...
    {                                                                                                              \
        ...
        webrtc::metrics::Histogram* histogram_pointer = rtc::AtomicOps::AcquireLoadPtr(&atomic_histogram_pointer); \
        if (!histogram_pointer)                                                                                    \
        {                                                                                                          \
            histogram_pointer = factory_get_invocation;                                                            \
            webrtc::metrics::Histogram* prev_pointer = rtc::AtomicOps::CompareAndSwapPtr(                          \
                &atomic_histogram_pointer,                                                                         \
                static_cast<webrtc::metrics::Histogram*>(nullptr),                                                 \
                histogram_pointer);                                                                                \
            RTC_DCHECK(prev_pointer == nullptr || prev_pointer == histogram_pointer);                              \
        }                                                                                                          \
        if (histogram_pointer)                                                                                     \
        {                                                                                                          \
