...
StringToNumber(const char* str, int base = 10)
{
    ...
    static_assert(
        std::numeric_limits<T>::max() <= std::numeric_limits<signed_type>::max() && std::numeric_limits<T>::lowest() >= std::numeric_limits<signed_type>::lowest(),
        "StringToNumber only supports signed integers as large as long long int");
    absl::optional<signed_type> value = string_to_number_internal::ParseSigned(str, base);
...
StringToNumber(const char* str, int base = 10)
{
    ...
    absl::optional<unsigned_type> value = string_to_number_internal::ParseUnsigned(str, base);
