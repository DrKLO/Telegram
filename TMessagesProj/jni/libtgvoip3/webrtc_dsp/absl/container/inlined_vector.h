...
class InlinedVector
{
    ...
    using DisableIfIntegral = absl::enable_if_t<!std::is_integral<Iterator>::value>;
...
public:
    ...
    InlinedVector(InlinedVector&& v) noexcept(
        absl::allocator_is_nothrow<allocator_type>::value || std::is_nothrow_move_constructible<value_type>::value);
    ...
    {
        if (ABSL_PREDICT_FALSE(this == &other))
            return *this;
    ...
    {
        const auto s = size();
        if (ABSL_PREDICT_FALSE(!allocated() || s == capacity()))
            return;
...
private:
    ...
    {
    public:
        explicit AllocatorAndTag(const allocator_type& a)
            : allocator_type(a)
        {
        }
    ...
    reference GrowAndEmplaceBack(Args&&... args)
    {
        ...
        reference new_element = Construct(new_allocation.buffer() + s, std::forward<Args>(args)...);
    ...
    union Rep {
        using ValueTypeBuffer = absl::aligned_storage_t<sizeof(value_type), alignof(value_type)>;
        using AllocationBuffer = absl::aligned_storage_t<sizeof(Allocation), alignof(Allocation)>;
    ...
    -> std::pair<iterator, iterator>
{
    ...
    else
    {
        ...
        size_type old_elements_in_used_space = slots_in_used_space - new_elements_in_used_space;
