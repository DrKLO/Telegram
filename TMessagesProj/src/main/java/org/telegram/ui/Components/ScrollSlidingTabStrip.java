...
public class ScrollSlidingTabStrip extends HorizontalScrollView {
    ...
    public StickerTabView addStickerIconTab(int id, Drawable drawable) {
        ...
        if (tab != null) {
            ...
            tab = new StickerTabView(getContext(), true);
    ...
    public View addStickerTab(TLObject thumb, SvgHelper.SvgDrawable svgThumb, TLRPC.Document sticker, TLRPC.TL_messages_stickerSet parentObject) {
        ...
        if (tab != null) {
            ...
            tab = new StickerTabView(getContext(), false);
            tab.setFocusable(true);
            tab.setOnClickListener(v -> delegate.onPageSelected((Integer) v.getTag(R.id.index_tag)));
            tabsContainer.addView(tab, position);
        }
...
//        }
        ...
        if (indicatorHeight >= 0) {
            ...
            switch (type) {
                case LINE:
                    ...
                    break;
                case TAB:
                    indicatorDrawable.setBounds((int) lineLeft + AndroidUtilities.dp(6), (int) (height - AndroidUtilities.dp(3)), (int) (lineLeft + width - AndroidUtilities.dp(6)), (int) (height));
                    break;
            }
    ...
    public boolean checkLongPress(MotionEvent ev) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_MOVE && draggingView != null) {
            ...
            if (currentDragPosition != p && canSwap(currentDragPosition, p)) {
                for (int i = 0; i < tabsContainer.getChildCount(); i++) {
...
//                ValueAnimator animator = swapViewsAnimators.get(view);
//                if (animator != null) {
//                    animator.removeAllListeners();
//                    animator.cancel();
//                }
//                animator = ValueAnimator.ofFloat((p - currentDragPosition) * getTabSize(), 0);
//                animator.addUpdateListener(valueAnimator -> {
//                    stickerTabView.dragOffset = (float) valueAnimator.getAnimatedValue();
//                    invalidate();
//                });
...
//                        swapViewsAnimators.remove(view);
//                        stickerTabView.dragOffset = 0;
//                        invalidate();
//                    }
//                });
//                swapViewsAnimators.put(view, animator);
//                animator.start();
                invalidate();
            }
    ...
    private boolean canSwap(int currentDragPosition, int p) {
        if (p >= 0 && p < 3) {
            return false;
        }
        return true;
    ...
    Runnable scrollRunnable = new Runnable() {
        @Override
        public void run() {
            int dx = AndroidUtilities.dp(4) * (scrollRight ? 1 : -1);
            scrollBy(dx, 0);
