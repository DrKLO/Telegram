...
public class GradientBackgroundView extends GLTextureView {
    ...
    private final Interpolator defaultInterpolator = new LinearInterpolator();
    private final long defaultDuration = 500L;
    ...
    @Nullable
    private GradientBackgroundListener listener;
    ...
    public void setListener(@Nullable GradientBackgroundListener listener) {
        this.listener = listener;
    }

    public void setPointsDeviation(@FloatRange(from = -1.0, to = 1.0) float deviation) {
        if (animator != null && animator.isRunning()) {
            return;
        }
        int prevPointsPosition = getPrevPosition();
        int nextPointsPosition = getNextPosition();
        for (int i = 0; i < AnimationsController.backPointsCount; ++i) {
            float xCurr = AnimationsController.getBackgroundPointX(currentPointsPosition, i);
            float yCurr = AnimationsController.getBackgroundPointY(currentPointsPosition, i);
            if (deviation < 0) {
                float xPrev = AnimationsController.getBackgroundPointX(prevPointsPosition, i);
                float yPrev = AnimationsController.getBackgroundPointY(prevPointsPosition, i);
                xCurr = xCurr + (xCurr - xPrev) * deviation;
                yCurr = yCurr + (yCurr - yPrev) * deviation;
            } else {
                ...
                xCurr = xCurr + (xNext - xCurr) * deviation;
                yCurr = yCurr + (yNext - yCurr) * deviation;
            }
            setPointPosition(i, xCurr, yCurr);
            setAnimationStartPoint(i, xCurr, yCurr);
        }
        invalidate();
    }
    ...
    public void startAnimation(boolean animateForward) {
        int nextPosition = animateForward ? getNextPosition() : getPrevPosition();
    ...
    private void startAnimation(int nextPointsPosition) {
        ...
        animator.setDuration(settings == null ? defaultDuration : settings.maxDuration);
        Interpolator interpolator = settings == null ? defaultInterpolator : settings.getInterpolator();
    ...
    private int getPrevPosition() {
        int pos = currentPointsPosition - 1;
        if (pos < 0) {
            pos = AnimationsController.backPositionsCount - 1;
        }
        return pos;
    }

    private int getNextPosition() {
        return (currentPointsPosition + 1) % AnimationsController.backPositionsCount;
    }


    public interface GradientBackgroundListener {
