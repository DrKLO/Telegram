...
public sealed class TlGen_StarsTransaction : TlGen_Object {
  public data class TL_starsTransaction(
    ...
    public override fun serializeToStream(stream: OutputSerializedData) {
      ...
      multiflags_5?.transaction_date?.let { stream.writeInt32(it) }
      multiflags_5?.transaction_url?.let { stream.writeString(it) }
      ...
      multiflags_17?.starref_peer?.serializeToStream(stream)
      multiflags_17?.starref_amount?.serializeToStream(stream)
      paid_messages?.let { stream.writeInt32(it) }
      premium_gift_months?.let { stream.writeInt32(it) }
      multiflags_23?.ads_proceeds_from_date?.let { stream.writeInt32(it) }
      multiflags_23?.ads_proceeds_to_date?.let { stream.writeInt32(it) }
    }
